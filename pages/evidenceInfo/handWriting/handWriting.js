// const wxml2canvas = require('../../common/wxml2canvas.js')
var app = getApp();
import {
  IMGURL
...
Page({
  ...
  onLoad() {
    ...
    console.log(app.globalData.signUrl)
    this.setData({
      web_src: app.globalData.signUrl
    })
    //获取当前时间
    var date = new Date();
    ...
    wx.createSelectorQuery().selectAll('#wrappers').boundingClientRect(function (rect) {
      that.setData({
        wid: rect[0].width,
        hei: rect[0].height,
      })
  ...
  setDraw() {
    let that = this;
    // const ctx1 = wx.createCanvasContext('handWriting') 
    // this.data.ctx.transform(1, Math.PI / 6, -Math.PI / 6, 1, 30, 10);
    // this.data.ctx.fillRect(50, 20, 100, 50);
    wx.canvasToTempFilePath({
      canvasId: 'handWriting',
      quality: 1,
      success(res) {
        // that.rotateBase64(res.tempFilePath)
        //   .then((img) => {
        //     console.log(`旋转后的base64图片：${img}`)
        // });
        that.setData({
          nowUrl: res.tempFilePath,
          hasWrting: true,
          writing: true,
        })
      }
  ...
  imageLoad(e) {
    console.log(e)
    var $width = (e.detail.width / 2);
    var $height = (e.detail.height / 2);
  ...
  toImage() {
    ...
    let widths;
    let heights;
    var obj = wx.createSelectorQuery("#wrappers");
    wx.createSelectorQuery().selectAll('#wrappers').boundingClientRect(function (rect) {
      that.setData({
        wid: rect[0].width,
        hei: rect[0].height,
    ...
    const wrapperId = '#wrappers'
    const drawClassName = '.draw'
    const canvasId = 'canvas'
    wxml2canvas(wrapperId, drawClassName, canvasId).then((r) => {
      console.log(r)
      wx.canvasToTempFilePath({
        canvasId: 'canvas',
        quality: 1,
