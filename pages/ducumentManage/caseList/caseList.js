...
Page({
  ...
  nextStep() {
    ...
    this.data.caseList.map(item => {
      if (item.selected) {
        flag = 1;
        app.globalData.caseId = item.lawCaseId;
        app.globalData.caseNo = item.sqCaseNo;
      }
    })
    if (flag === 1) {
      wx.navigateTo({
        url: `/pages/ducumentManage/ducumentList/ducumentList`,
      })
    } else {
      wx.showToast({
        title: '请选择案件',
    ...
    selectCase(data).then(res => {
      ...
      if (res.state == 100) {
        ...
        for (let i = 0; i < res.result.content.length; i++) {
          res.result.content[i].selected = false;
