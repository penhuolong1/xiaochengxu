...
Page({
  ...
  onLoad(option) {
    ...
    if (option.q) {
      ...
      let src = decodeURIComponent(option.q)
      console.log(1)
      this.dealUrl(src)
    }
    if (option.checkAry){
      this.setData({
        checkAry: option.checkAry.split(',')
      })
    }
    let that = this
  ...
  setDraw() {
    ...
    wx.canvasToTempFilePath({
      ...
      success(res) {
        //图片旋转
        let iinmg = res.tempFilePath;
        let canvass = wx.createCanvasContext('canvas');
        var width = 170
        var height = 340
        canvass.translate(0, width);
        canvass.rotate(270 * Math.PI / 180);
        canvass.drawImage(iinmg, 0, 0, width, height);
        canvass.draw(false);
        wx.showLoading({
          title: '提交中… '
        })
          ...
          wx.canvasToTempFilePath({
            ...
            success:  (ress) => {
              ...
              wx.getFileSystemManager().readFile({
                filePath: ress.tempFilePath, //选择图片返回的相对路径
                encoding: 'base64', //编码格式
                success: res => {
                  if (!that.data.isQrcode) {
                    //接口提交
                    tableFileAddSign({
                      fileIds: that.data.checkAry,
                      image: res.data
                    }).then(res => {
                      if (res.state == 100) {
                        wx.hideLoading()
